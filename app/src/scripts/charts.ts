...
function init() {
    const ws = new WebSocket(`ws://192.168.43.188/ws`);
    ...
    ws.onmessage = function (event)
    {
        ...
        for (let i = 0; i < 10; i++)
        {
            switch (dataset) {
                case 'Temperature':
                    arrayofData.push(parseInt(objJson1[i].Temperature));
                    break;
                case 'Humidity':
                    arrayofData.push(parseInt(objJson1[i].Humidity));
                    break;
                case 'Altitude':
                    arrayofData.push(parseInt(objJson1[i].Altitude));
                    break;
                case 'Pressure':
                    arrayofData.push(parseInt(objJson1[i].Pressure));
                    break;
                case 'Light Level':
                    arrayofData.push(parseInt(objJson1[i].LightLevel));
                    break;
            }
        }
